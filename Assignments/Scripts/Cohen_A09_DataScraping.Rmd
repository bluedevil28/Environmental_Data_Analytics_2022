...
```{r fetch.and.plot.Durham.Asheville.2015.data.compare}
#8
asheville_df <- scrape.it(2015,'01-11-010')

combined_df <- rbind(durham_2015, asheville_df)

ggplot(combined_df,aes(x=Date,y=Max_Withdrawals_mgd,color=Water.System.Name)) + 
  geom_line() +
  geom_smooth(method="loess",se=FALSE) +
...
ggplot(df_single_asheville_years,aes(x=Date,y=Max_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
