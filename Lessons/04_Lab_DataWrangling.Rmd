...
NTL.phys.data.Tuesday.1990thru1999 <- NTL.phys.data %>%
...
NTL.phys.data.Tuesday.1990thru1999.July <- NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>%
...
NTL.phys.data.Tuesday.1990thru1999.July.summaries <- 
  NTL.phys.data.Tuesday.1990thru1999.July %>%
  filter(depth == 0) %>%
...
NTL.phys.data.irradiance.gathered <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck,
                                    names_to = "irradiance", values_to = "shinyness")

#Exercise 6: spread temperatureC into more than one column based on the depth

NTL.phys.data.temperature.spread <-
  pivot_wider(NTL.phys.data, names_from = temperature_C, values_from = depth)
