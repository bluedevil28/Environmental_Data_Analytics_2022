...
#Tidy the data
gage_data <- gage_data %>% 
  dplyr::select(datetime, discharge_cfs,gage_ht_ft) %>% 
...
```{r pull.and.plot.discharge.data}
#Set the URL
theURLtwo <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
gage_data_two <- read.table(theURLtwo,skip = 28,header=TRUE,sep='\t') 

#Update the column headers
colnames(gage_data_two) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
gage_data_two <- gage_data_two %>% 
  dplyr::select(datetime, discharge_cfs,gage_ht_ft) %>% 
  dplyr::mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data_two,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
neuse_by_year <- neuse %>% 
  ...
  dplyr::summarise(mean_annual_flow = mean(Flow))
...
the_df <- data.frame(
  ...
  "Capacity" = as.numeric(gsub(",","",the_capacity)), #gsub to replace comma with nothing
...
#map is from tidyverse/purrr package
```
