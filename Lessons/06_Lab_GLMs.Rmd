...
> mean relation to 50; ozone levels & safety
...
# --> means are different, or not enough data to prove they are the same
...
pm.by.ozone.regression <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(pm.by.ozone.regression)

#rsquare .1316

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 
pm.ozone.scatter <- ggplot(EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_smooth(method = "lm") + 
  geom_point() +
  geom_smooth(method = "lm",col)
print(pm.ozone.scatter)
...
OzonebyALL.regression <- lm(data = EPAair.subset, PM2.5 ~ Ozone + Year + Month + 
                            meanLng + meanLat)
summary(OzonebyALL.regression)

step(OzonebyALL.regression)

#Exercise 7: Run another regression using the variables selected on Exercise 6. Compare r-squared value with the one from Exercise 6.
